			...
			if (err) {
				...
				api.getAssignmentsForAllClasses(teacherID, function(err, assignments){
					if (err) {
						...
						if(!assignments){
							assembledFile = data.replace('<div id="appendhere">', '<p class="first-time"><strong>Welcome to JustOpine!</strong> <br> To get started, click on <em>Class Lists</em> to add a class. <br> Once you\'ve done that, you are free to set your class their first assignment.</p>');
						} else {
							for (var key in assignments) {
