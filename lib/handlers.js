			...
			if (err) {
				...
				api.getPupils(userID, className, function(err, pupils) {
					...
					for (var i = 0; i < pupils.length; i++){
						...
						pupilList += '<td><img class="student-icon" src="../static/public/images/face.png"></td>' + '<td>' + JSON.parse(pupils[i]).firstname + '</td><td>' + JSON.parse(pupils[i]).surname + '</td><td>' + JSON.parse(pupils[i]).level + '</td></tr>';
					}
