				...
				api.getAssignmentsForAllClasses(teacherID, function(err, assignments){
					if (err) {
						...
						if(!assignments){
							...
							assembledFile = data.replace("<div id='appendhere'>", "<div class='first-time'><h3>Welcome to JustOpine!</h3> <br> <p>You have no active assignments.<br><br>If this is your first time here, click on <em>Class Lists</em> to add your first class.</p></div>");
						} else {
							for (var key in assignments) {
			...
			if (err) {
				...
				api.getClasses(teacherID, function(err, classes){
					if (err){
						...
						if (!classes){
							assembledFile = data.replace("<div id='appendhere'>", "<div class='first-time'><h3>You have no classes.</h3><br>  Click <em>Add Class</em> to register your first class.<br>Once you've done that, click on the class to add pupils to it.</p></div>");
						} else {
		...
		fs.readFile("./public/views/pupils.html", "utf8", function (err,data) {
			if (err) {
				...
				api.getPupils(userID, className, function(err, pupils) {
					...
					assembledFile = data.replace("<tr id='appendhere'></tr>", pupilList);
					reply(assembledFile);
				});
			}
		});
