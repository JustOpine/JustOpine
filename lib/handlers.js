	...
	'dashboardTeacher' : function (request, reply){
		...
		fs.readFile('./public/views/dashboard-teacher.html', 'utf8', function (err,data) {
			if (err) {
				...
				api.getClasses(teacherID, function(err, classesArray){

					classesArray.map(function(className) {

						api.getAssignmentsForClass(teacherID, className, function(err, assignments) {
							...
							var assembledFile = data.replace('<div id="appendhere"></div>', assembledData);
							reply(assembledFile);
