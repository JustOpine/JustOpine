...
var handlers = {
	...
	"dashboardTeacher" : function (request, reply){
		console.log("auth'd?", request.auth);
		var assembledData = "";
		var assembledFile;
			...
			if (err) {
				...
				api.getClasses(teacherID, function(err, classes){
					if (err){
						...
						if (classes.length === 0){
							assembledFile = data.replace("<div id='appendhere'>", "<div class='first-time'><h3>You have no classes registered.</h3><br>  Click <em><strong>Add Class</strong></em> to register your first class.<br>Once you've done that, click on the class to add pupils to it.</p></div>");
						} else {
			...
			if (err) {
				...
				api.getClasses(userID, function(err, classNames){
					if (classNames.length === 0) {
						console.log("cant set an assignment if you have no classes!");
					}
					var assembledData = "";
