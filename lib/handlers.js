	...
	return {
		'home' : function (request, reply){
			if (request.auth.isAuthenticated) {
				if(request.auth.session.isTeacher){
					return reply.redirect('/teacherdash');
				//} else if(request.auth.session.isAdmin){
					// return reply.redirect('/editadash')
					// return reply('This would be Edita\'s page (once we build it));
				} else { //must be student then
					return reply.redirect('/studentdash');
				}
			} else {
		...
		'login': function(request, reply){
			...
			if (request.method === 'get' || failureMessage) { // if coming to page for 1st time || failed login attempt
