...
var handlers = {
	"home": function (request, reply) {
		console.log("home auth'd?", request.auth.isAuthenticated);
	...
	"login": function(request, reply){
		console.log("login auth'd?", request.auth.isAuthenticated);
	...
	"api": function(request, reply) {
		...
		if (path === "addTeacher") {
			...
			if (path === "addClass") {
				...
				console.log("newPupilInfo", newPupilInfo);
				api.addPerson(newPupilInfo, function(err, data){
					if (err) {
						reply.redirect("/error");
					} else {
						reply.redirect("/pupils?" + className); // reply to addUser.js
					}
				});
				...
				if (!response.threeWords || !response.response){
