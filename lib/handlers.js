...
var handlers = {
	...
	'api' : function(request, reply) {
		...
		if (path === 'addClass') {
			...
			var referrerPath = request.info.referrer.split('/')[3];
			...
			if (request.auth.credentials.isTeacher = true) {
				...
				api.getAssignmentsForAllClasses(teacherID, function(err, assignments){
					...
					for (var i = 0; i < assignments.length+1; i++){
						if (i === assignments.length){
							console.log(i, assignments.length);
	...
	'pupils' : function (request, reply){
		if (request.raw.req.statusCode === null) {
			reply.redirect('/error');
		}
