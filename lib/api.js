...
module.exports = {
    ...
    store : function (userInfo, passHash, id, callback) {
      // store id in allUserIDs
        ...
        client.hset(key2, finalID, userInformation, function(err, data) {
            if(err){
                callback(err, null);
            } else {
    ...
    getClasses : function(teacherID, callback) {
        console.log("in getClasses", teacherID);
        var setKey = teacherID + ':' + 'class';
    ...
    addAssignment : function (teacherID, newAssignmentInfo, callback) {
        ...
        this.generateID(newAssignmentInfo, function (err, assignmentID) {
            if(err){
                ...
                console.log('newAssignmentInfo', newAssignmentInfo);
                // save key to a set of discussions
                this.client.hset(setKey, assignmentID, JSON.stringify(newAssignmentInfo), function(err, data) {
                    client.quit();
                    if(err){
                        callback(err, null);
                    } else if (data === 0){
    ...
    getAssignmentsForAllClasses : function (teacherID, callback) {
        console.log("in getAssignmentsForAllClasses", teacherID);
            ...
            classesArray.forEach(function(e) {
                ...
                return client.hgetall(setKey, function(err, data) {
                    for (var assignmentID in data){
                        data[assignmentID] = JSON.parse(data[assignmentID]);
