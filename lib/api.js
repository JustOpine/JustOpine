...
module.exports = {
    ...
    addPerson : function (userInfo, callback) {
        ...
        this.generateID(userInfo, function (err, userInfo) {
            if(err){
                callback(err, null);
            } else {
                this.encryptPassword(userInfo, callback);
            }
    ...
    encryptPassword : function (userInfo, callback) {
        bcrypt.genSalt(10, function(err, salt) {
            bcrypt.hash(userInfo.password, salt, function(err, passHash) {
                // Store hash in your password DB.
                this.store(userInfo, passHash, function(err, verdict){
                    if (err) {
                        callback(err, null);
                    } else {
    ...
    store : function (userInfo, passHash, callback) {

        // set users's key
        // var key = (userInfo.role || userInfo.teacherID) + ':' + id;
        var key = (userInfo.isTeacher ? 'teacher' : 'pupil');
