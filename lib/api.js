...
module.exports = {
    ...
    generateID : function (userInfo, callback) {
        ...
        // must check if id exists in teacher list or pupil list.
    ...
    store : function (userInfo, passHash, id, callback) {
        ...
        // STORING ALL TEACHERS, STORING ALL pupilsData
        // set user's key.  teacher --> 'teacher:xxxxxx' pupil ---> '[teacherid]:xxxxxx'
        var key1 = (userInfo.isTeacher ? 'teacher' : userInfo.teacherID) + ':' + id;
        var key2;
        ...
        */
        ...
        // add user's id to their set
        if(userInfo.className){
            key2 = userInfo.teacherID + ':' + userInfo.className;
        } else {
            key2 = 'teacher';
        }
        this.client.sadd(key2, id, function(err, data) {
            if(err){
                callback(err, null);
            } else {
                console.log("successfully added to set", key2, id);
            }
        });
        ...
        this.client.hset('login', userInfo.username, id, function(err, data) {
            ...
            this.client.quit();
            console.log('added to login hash', userInfo.username, id);
