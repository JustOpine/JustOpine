...
module.exports = {
    ...
    getAssignmentsForAllClasses : function (teacherID, callback) {
        this.getClasses(teacherID, function(err, classesArray) {
            var counter2 = 0;
            var assignmentData = [];
            console.log("counter2 at start", counter2)
                ...
                this.client.smembers(setKey, function(err, assignmentKeys) {
                    ...
                    assignmentKeys.forEach(function(assignmentKey) {
                        return client.hgetall(assignmentKey, function(err, data) {
                            ...
                            console.log("counter2 in middle", counter2);

                            assignmentData.push(data);
                            ++counter;
                            if(counter2 === classesArray.length && counter === assignmentKeys.length) {
                                console.log("inside if statement, counter2", counter2, "classArray.length", classesArray.length);
                                console.log("inside if statement, counter1", counter, "assignmentKeys.length", assignmentKeys.length);
