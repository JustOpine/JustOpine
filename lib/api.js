...
module.exports = {
    ...
    getPupils : function(teacherID, className, callback) {
        var pupilsArray = [];
        var setKey = teacherID + ':' + className + ':' + 'class';
        console.log('teacherid',teacherID);
        console.log('whole setkey',setKey);
        this.client.hgetall(setKey, function(err, pupilsInClass) {
            console.log(pupilsInClass);
            // get value of each (unknown) key
            for (var key in pupilsInClass) {
                pupilsArray.push(pupilsInClass[key]);
            }
            callback(null, pupilsArray);
        });
    },
