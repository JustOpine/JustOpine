...
module.exports = {
    ...
    addResponse: function (timestamp, className, assignmentID, response, userName, userID, teacherID, callback) {
        ...
        this.generateID(response, function (err, id) {
            if(err){
                ...
                client.zrange(setKey, 0, -1, function(err, responseIDs){
                    if(err){
                        callback(err, null);
                    } else {
                        this.responseAlreadyExists(responseIDs, userID, function(matchingResponses){
                            if(matchingResponses.length === 0){
                                client.zadd(setKey, Number(timestamp), hashKey, function(err, data) {
                                    if(err){
                                        ...
                                        // client.hset(hashKey, "userID", userID);
                                        //
    ...
    responseAlreadyExists: function(responseIDs, userID, callback){
        var responseIDsLength = responseIDs.length;
        if (responseIDsLength === 0){
            callback([]);
        } else {
            var duplicates = 0;
            for (var i = 0; i < responseIDsLength; i++){
                client.hget(responseIDs[i], "userID", function(err, storedID){
