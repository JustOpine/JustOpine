...
module.exports = {
    ...
    generateID : function (userInfo, callback) {
        ...
        // pupil --> [teacherid]:[className]:[pupilID]
    ...
    store : function (userInfo, passHash, id, callback) {
      // store id in allUserIDs
        ...
        // set user's key/id.  teacher --> 'xxxxxx' pupil ---> '[teacherid]:[className]:xxxxxx'
        var finalID = (userInfo.isTeacher ? id : userInfo.teacherID + ':' + userInfo.className + ':' + id);
        var key2;

        // save id in userInfo obj - key is either 'teacherID' or 'pupilID'
        ...
        this.client.hset('login', userInfo.username, finalID, function(err, data) {
            if(err){
                callback(err, null);
            }
            this.client.quit();
        ...
        // remove password from userInfo obj, then add to DB
        ...
        */
        ...
        // add user's id to their hash
        // hashname: 'teachers' || [teacherid]:[className]:class
        ...
        this.client.hset(key2, finalID, JSON.stringify(userInfo), function(err, data) {
            if(err){
                callback(err, null);
            } else {
                console.log("successfully added to set", key2, finalID, JSON.stringify(userInfo));
            }
        });
